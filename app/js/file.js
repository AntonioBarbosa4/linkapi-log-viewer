...
module.exports = class LogFile {
  ...
  transform() {
    ...
    const data = groupsExecutionId
      .sort((a, b) => {
        if (a[0] && b[0] && a[0].timestamp && b[0].timestamp) {
          return isBefore(parseISO(a[0].timestamp), parseISO(b[0].timestamp))
            ? 1
            : -1;
        }
        return -1;
      ...
      .map((g) => {
        ...
        return groupUniqueKey
          ...
          .sort((a, b) => {
            if (a.uniqueKey && a.uniqueKey.startsWith('session')) {
              if (a.firstTime && b.firstTime && isBefore(parseISO(a.firstTime), parseISO(b.firstTime))) {
                return -1;
              }
              return 1;
            }
